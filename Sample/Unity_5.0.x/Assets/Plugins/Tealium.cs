...
[DllImport ("__Internal")]
private static extern void Tealium_TrackEvent(IntPtr evt, string eventType, string eventTitle);

#elif UNITY_ANDROID && !UNITY_EDITOR

private static AndroidJavaClass _tealiumClass = new AndroidJavaClass("com.tealium.library.Tealium");
private static AndroidJavaClass _tealiumUnityClass = new AndroidJavaClass("com.tealium.unityplugin.UnityApplication");
    ...
    private static void TrackCustomEvent(string name, string key, string value, Dictionary<string, string> data)
    {
        ...
        if (value == null)
        {
            if (key == "link_id")
            {
                Debug.LogError("A name must be provided when calling Tealium.TrackEvent(...). Call will be ignored.");
            }
            else
            {
                Debug.LogError("A name must be provided when calling Tealium.TrackView(...). Call will be ignored.");
            }
            return;
        }
    ...
    AndroidJavaObject instance = _tealiumClass.CallStatic<AndroidJavaObject>("getInstance", "tealium_main");
    instance.Call("trackEventType", name, value, map);
	...
	Tealium_TrackEvent(eventPtr, name, value);			
